...
that was assembled on the host (see `install.sh <../install.sh>`_). That image is
...
the kernel a pointer to it (see `main.rst <main.rst>`_ ``systemModule()``), so the
...
- `drivers/ata.rst <drivers/ata.rst>`_ — the disk ``write()`` primitive used here.
- `auth.rst <auth.rst>`_ — the credential format the installed system logs in with.
