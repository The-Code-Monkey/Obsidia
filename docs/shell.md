...
### Login
- `login()` — gates the shell behind a username + password when a credential exists. Reads `user:phc` from the auth module (or `/OBSIDIA/AUTH` on a plain FAT32 disk via [`loadCredential`]), prompts for a username and a masked password (`readLine`), and verifies the password with [`auth.md`](auth.md) (scrypt). Loops until correct; no credential means an open shell.
