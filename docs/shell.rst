...
- ``run()`` — runs ``login()`` first (if a credential is configured), then the shell loop; ``noreturn``. Blinks the on-screen cursor, drains the input ring, and ``hlt``\ s when idle.
...
- ``setAuthModule(m)`` — receives the login credential Limine loaded as a module (called by ``main`` before the shell starts); preferred over the on-disk file.

Login
-----

- ``login()`` — gates the shell behind a username + password when a credential exists. Reads ``user:phc`` from the auth module (or ``/OBSIDIA/AUTH`` on a plain FAT32 disk via `loadCredential`), prompts for a username and a masked password (``readLine``), and verifies the password with `auth.rst <auth.rst>`_ (scrypt). Loops until correct; no credential means an open shell.
...
- ``execute(raw)`` — trims, splits command word from args, and dispatches. Built-ins: ``help``, ``clear``, ``echo <text>``, ``mem``, ``uptime``, ``history``, ``ps``, ``ls [path]``, ``cat <path>``, ``exec <path>`` (run an ELF/flat binary), ``install`` (clone the system image onto the disk; shown only on the installer medium), ``sleep``, ``restart``/``reboot``, ``shutdown``/``poweroff``, ``crash``.
