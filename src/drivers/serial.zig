...
const config = @import("config"); // build-time flags (debug_log) generated by build.zig
...
// Like print(), but ONLY emitted when the kernel was built with -Ddebug-log=true.
// Use this for verbose boot / init / self-test DIAGNOSTICS — the "[GDT] ...",
// "[PMM] ...", "[SCHED] ... self-test ..." subsystem markers. They're invaluable
// for the test harness and for debugging, but they make an ordinary boot noisy.
// When config.debug_log is false (the default) the comptime-known `return` makes
// the whole body dead code, so nothing is emitted and no formatting work happens.
// Always-on UI — shell output, prompts, login, user-visible errors — keeps using
// print()/note() so a normal boot still works exactly as before.
pub fn log(comptime format: []const u8, args: anytype) void {
    if (!config.debug_log) return; // comptime-false → dead-code-eliminated, zero cost
    print(format, args);
}
...
// The debug-gated form of note(): serial-only (no console mirror) AND only emitted
// when built with -Ddebug-log=true. For diagnostics that must NOT re-enter the
// framebuffer console — e.g. the console's own scrollback status lines, which call
// this from inside the console code. Compiles away entirely when debug_log is false.
pub fn logNote(comptime format: []const u8, args: anytype) void {
    if (!config.debug_log) return; // comptime-false → dead-code-eliminated
    note(format, args);
}
